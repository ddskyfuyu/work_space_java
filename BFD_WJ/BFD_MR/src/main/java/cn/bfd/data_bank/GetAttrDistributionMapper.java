 ...
 */
public class GetAttrDistributionMapper extends Mapper<LongWritable,Text,Text, IntWritable>{

	private static Logger LOG = Logger.getLogger(GetAttrDistributionMapper.class);
...
//			}
		...
		if(!objMap.containsKey("gid")){
			...
			objMap.put("gid", UpUtils.reverseString(rowkey));
		}
	...
	public void getUpAttributes(List<TGet> gets, List<String> gids, Context context) throws InterruptedException,IOException{
		...
		while(retryCount < 5){
			try {
				...
				//int hbaseNumber = gets.size();
				//int gidNumber = gids.size();
				//int failedNumber = fieldSet.size();
				//int findHitNumber = hbaseNumber - failedNumber ;

				//LOG.info("First Get From Hbase [Total: " + gidNumber + ", Finish:" + findHitNumber + ", failed:" + failedNumber + ",hbase result:" + hbaseNumber +"]" );
				break;
			} catch (Exception  e) {
