...
public class GetGidBaseCid {
     ...
     public static class MyMapper extends TableMapper<Text, Text>{
         ...
         public void map(ImmutableBytesWritable row, Result val, Context context) throws IOException, InterruptedException{
             ...
             if(family.equals("-1") || cid.equals("-1")){
            	   System.out.println("FAMILY, CID key is wrong");
            	   return;
               }   
             try{
            	 UserProfile2.UserProfile up = UserProfile2.UserProfile.parseFrom(val.getColumnLatest(Bytes.toBytes(family),Bytes.toBytes(column)).getValue());
            	 if(up != null){
            		 for(int i = 0; i < up.getCidInfoCount(); ++i){
            			 if(up.getCidInfo(i).getCid().equals(cid)){
            				 String cid_info = JsonFormat.printToString(up.getCidInfo(i));
            				 context.write(new Text(up.getUid()), new Text(String.valueOf(cid_info.length())));
     ...
     public static void main(String[] args) throws Exception{
          ...
          if(otherArgs.length != 4){
       	   System.out.println("Usage: <Table>, <FAMILY> <output> <cid>");
       	   return;
          }
          conf.set("Table", otherArgs[0]);
          conf.set("FAMILY", otherArgs[1]);
          conf.set("CID", otherArgs[3]);
          ...
          Job job = new Job(conf, "GetUpBaseCid");
          job.setJarByClass(GetGidBaseCid.class);
